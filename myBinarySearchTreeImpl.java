...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public int my_count_at_level(int level){
		...
		if(my_is_empty()){
			...
			if(level!=1){
				count+= my_left_tree().my_count_at_level(level-1);
				count+= my_right_tree().my_count_at_level(level-1);
				return count;
			}
			else{
				return 1;
	...
	public boolean my_is_balanced(){
		...
		if(my_is_empty()){
			...
			left_length += my_left_tree().my_length();
			right_length += my_right_tree().my_length();
			if (left_length == right_length || left_length == right_length+1 || left_length == right_length-1){
				ans = true;	
			}	
			if(ans==true){
				boolean compare1;
				boolean compare2;
				compare1 = my_left_tree().my_is_balanced();
				compare2 = my_right_tree().my_is_balanced();
	...
	public int my_count_smaller_nodes(T1 key){
		...
		if(my_is_empty()){
			...
			if (key.compareTo(this.my_root().getKey()) >0){
				count+= my_left_tree().my_count_smaller_nodes(key) +1;
	...
	public int my_find_node_at_level(T1 key){
		...
		if(my_is_empty()){
			...
			else if(key.compareTo(this.my_root().getKey()) >0){
